...
function SidebarNav({ activePath, onNavigate, user }: { activePath: string; onNavigate?: () => void; user: any }) {
  ...
  const links: SidebarLink[] = [
    { href: "/executive/dashboard", label: "แดชบอร์ดผู้บริหาร", icon: LayoutDashboard },
...
export default function ExecutiveShell({
  ...
  const displayName = user ? `${user.firstName} ${user.lastName}`.trim() : "ผู้บริหาร";
  const subtitle = user?.position || user?.faculty || "ผู้บริหารคณะวิทยาศาสตร์";
      ...
      >
        <div className="mb-5 flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-blue-100">ผู้บริหาร</p>
          </div>
          <button
            type="button"
            aria-label="ปิดเมนู"
