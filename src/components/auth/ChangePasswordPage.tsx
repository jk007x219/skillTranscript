...
export default function ChangePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => {
        router.push(user?.role === "student" ? "/student/dashboard" : "/teacher/students");
      }, 2000);
    } catch (err) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#F8FAFC] via-blue-50 to-sky-100 px-4">
        <div className="max-w-md w-full rounded-xl border border-emerald-100 bg-emerald-50 p-8 text-center">
          ...
          <h2 className="mt-4 text-2xl font-semibold text-emerald-700">เปลี่ยนรหัสผ่านสำเร็จ</h2>
          <p className="mt-2 text-sm text-emerald-600">{message}</p>
          <p className="mt-4 text-sm text-slate-500">กำลังนำคุณไปยังหน้าแดชบอร์ด...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#F8FAFC] via-blue-50 to-sky-100 px-4 py-8">
      <div className="w-full max-w-md rounded-2xl border border-blue-100 bg-white p-8 shadow-lg">
        <div className="text-center">
          ...
          <h2 className="mt-4 text-2xl font-semibold text-[#1565C0]">เปลี่ยนรหัสผ่าน</h2>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            <label htmlFor="currentPassword" className="block text-sm font-medium text-[#1565C0]">
              รหัสผ่านปัจจุบัน
            </label>
