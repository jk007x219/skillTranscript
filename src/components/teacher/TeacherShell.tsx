...
function SidebarNav({ activePath, onNavigate, user }: { activePath: string; onNavigate?: () => void; user: any }) {
  ...
  const links = isExecutive
    ? [{ href: "/executive/dashboard", label: "แดชบอร์ดผู้บริหาร", icon: LayoutDashboard }, ...baseLinks]
    : baseLinks;
...
export default function TeacherShell({
  ...
  const isExecutive = user?.role === 'executive' || (user?.role === 'teacher' && user?.isExecutive === true);
  const displayName = user ? `${user.firstName} ${user.lastName}`.trim() : "อาจารย์";
  const subtitle = user?.position || user?.faculty || "อาจารย์ที่ปรึกษา";
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(255,193,7,0.16),transparent_28%),linear-gradient(135deg,#F8FAFC_0%,#EEF6FF_48%,#F8FAFC_100%)] text-slate-900">
        ...
        <div className="flex h-20 items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            ...
            >
              ...
              <Image src="/tsu-logo.png" alt="TSU Logo" width={260} height={84} priority className="h-10 w-auto" />
              <div className="hidden min-w-0 border-l border-slate-200 pl-3 md:block">
                <p className="truncate text-sm font-semibold text-slate-950">คณะวิทยาศาสตร์และนวัตกรรมดิจิทัล</p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="group relative">
              ...
              <div className="absolute right-0 top-full hidden w-44 overflow-hidden rounded-xl border border-blue-100 bg-white shadow-lg group-hover:block">
                ...
                <Link href="/change-password" className="flex items-center gap-2 px-4 py-3 text-sm text-slate-700 transition hover:bg-blue-50">
                  <Lock className="h-4 w-4" />
                  เปลี่ยนรหัสผ่าน
                </Link>
                {isExecutive && (
                  <Link href="/executive/dashboard" className="flex items-center gap-2 px-4 py-3 text-sm text-slate-700 transition hover:bg-blue-50">
        ...
        <div className="bg-amber-50 border-b border-amber-200 px-4 py-2.5 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            <div className="flex items-center gap-3 text-sm text-amber-700">
              ...
              <span>
                <span className="font-medium">⚠️ คุณจำเป็นต้องเปลี่ยนรหัสผ่าน</span>
                <span className="hidden sm:inline"> เพื่อความปลอดภัยของบัญชีของคุณ</span>
              </span>
            </div>
            <Link
      ...
      >
        <div className="mb-5 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-white">ระบบบริหารจัดการ</p>
            <p className="truncate text-xs text-blue-100">อาจารย์ที่ปรึกษา</p>
          </div>
          <button
            type="button"
            aria-label="ปิดเมนู"
        ...
        <SidebarNav activePath={activePath} user={user} onNavigate={() => setIsMobileMenuOpen(false)} />
      </aside>
      ...
      <div className="grid lg:grid-cols-[214px_1fr]">
        <aside className="sticky top-20 hidden h-[calc(100vh-5rem)] bg-gradient-to-b from-[#0D47A1] via-[#1565C0] to-[#0A3A83] p-4 shadow-[12px_0_40px_rgba(13,71,161,0.12)] lg:block">
          <SidebarNav activePath={activePath} user={user} />
        </aside>
        {children}
      </div>
    </main>
