...
// ในส่วน useEffect ที่โหลดข้อมูล dashboard
useEffect(() => {
  if (!user?.studentId) {
    setDashboard(null);
    return;
  ...
  fetch(`/api/students/${user.studentId}/dashboard`, { signal: controller.signal })
    ...
    .catch((fetchError) => {
      ...
      setError(fetchError instanceof Error ? fetchError.message : "ไม่สามารถโหลดข้อมูลคะแนนนิสิตได้");
      setDashboard(null);
    })
  ...
  const { facultySkillProgress, essentialSkillProgress } = useMemo(() => {
    ...
    const facultyItems = skills.filter((skill) => isFacultySkill(skill.title));
    const essentialItems = skills.filter((skill) => !isFacultySkill(skill.title));

    return {
      facultySkillProgress: facultyItems,
      essentialSkillProgress: essentialItems,
    };
  }, [dashboard]);
  ...
  const summaryCards = [
    {
      ...
      iconClassName: "bg-gradient-to-br from-[#1565C0] to-[#0D47A1] text-white",
    },
    {
      icon: CalendarDays,
      ...
      iconClassName: "bg-gradient-to-br from-[#FFC107] to-[#FF9800] text-slate-950",
    },
    {
      icon: FileBadge,
      ...
      iconClassName: "bg-gradient-to-br from-[#4AA3D8] to-[#1565C0] text-white",
    },
  ];
      ...
      <section className="space-y-6 p-4 sm:p-6 lg:p-7">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0D47A1] via-[#1565C0] to-[#4AA3D8] p-6 text-white shadow-[0_22px_60px_rgba(13,71,161,0.24)] lg:grid lg:grid-cols-[1fr_280px] lg:items-center lg:p-8">
          ...
          <div className="relative">
            <p className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-medium text-blue-50 ring-1 ring-white/20">
              <Sparkles className="h-4 w-4 text-[#FFC107]" aria-hidden="true" />
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {summaryCards.map((card) => (
            ...
            >
              <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl shadow-sm transition group-hover:scale-105 ${card.iconClassName}`}>
