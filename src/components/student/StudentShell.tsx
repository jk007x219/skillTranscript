...
const sidebarLinks: SidebarLink[] = [
  { href: "/student/dashboard", label: "แดชบอร์ด", icon: Home },
  { href: "/student/activities", label: "กิจกรรมทั้งหมด", icon: CalendarDays },
...
function SidebarNav({
  ...
  return (
    <>
      <div className="mb-5 rounded-xl border border-white/10 bg-white/10 p-3 text-white shadow-inner">
        <p className="text-[11px] text-blue-100">สถานะบัญชี</p>
        <p className="mt-1 text-sm font-semibold">พร้อมใช้งาน</p>
      </div>
          ...
          <div className="flex items-center gap-4">
            <div className="group relative">
              ...
              <div className="absolute right-0 top-full hidden w-44 overflow-hidden rounded-xl border border-blue-100 bg-white shadow-lg group-hover:block">
                <Link href="/student/profile" className="block bg-[#FFC107] px-4 py-3 text-sm font-semibold text-slate-950">
                  ตั้งค่าโปรไฟล์
                </Link>
      ...
      >
        <div className="mb-5 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-white">Skill Transcript</p>
            <p className="truncate text-xs text-blue-100">เมนูสำหรับนิสิต</p>
          </div>
          <button
            type="button"
            aria-label="ปิดเมนู"
        ...
        <SidebarNav activePath={activePath} onNavigate={() => setIsMobileMenuOpen(false)} />
      </aside>
