...
function RadarChartSVG({
  ...
  const point = (value: number, index: number, extra = 0) => {
    ...
    const r = (value / 100) * radius + extra;
...
export default function SkillTranscriptPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/students/${user.studentId}/skill-transcript`, {
      signal: controller.signal,
    })
      .then(async (response) => {
        const data = await response.json().catch(() => ({}));
        if (!response.ok) {
          throw new Error(data.message || "ไม่สามารถโหลดข้อมูล Skill Transcript ได้");
        }
        setTranscript(data);
      })
      .catch((fetchError) => {
        if (fetchError instanceof DOMException && fetchError.name === "AbortError") return;
        setError(fetchError instanceof Error ? fetchError.message : "ไม่สามารถโหลดข้อมูล Skill Transcript ได้");
        setTranscript(null);
      })
  ...
  const data = useMemo(() => {
    ...
    return {
      ...
      skillsBars2: personalSkills.map((skill) => ({
        name: skill.name,
        percent: skill.percent,
      })),
      skillsScores: transcript.skills.map((skill) => ({
        name: skill.name,
        score: `${formatScore(skill.earnedScore)}/${formatScore(skill.maxPossibleScore)}`,
      })),
    };
  }, [transcript]);
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 text-center text-sm text-red-600">
        {error || "ไม่พบข้อมูล Skill Transcript"}
      </div>
    );
  }
      ...
      <style jsx global>{`
        ...
        * {
        ...
        .transcript-card {
          ...
          box-shadow: 0 2px 10px rgba(32, 60, 99, 0.08);
          overflow: hidden;
        }
              ...
              <div className="transcript-card profile-card">
                <div className="flex flex-col items-center">
                  ...
                  <div className="w-full mt-[7px] space-y-[5px]">
                    <InfoRow
                      icon={<GraduationCap size={9} fill="#173b69" strokeWidth={1} />}
                      label="รหัสนิสิต"
                      value={data.profile.studentId}
                      inline
                    />
                    <InfoRow
                      icon={<Home size={9} fill="#173b69" strokeWidth={1} />}
                      label="คณะ"
                      value={data.profile.faculty}
                    />
                    <InfoRow
                      icon={<BookOpen size={9} fill="#173b69" strokeWidth={1} />}
                      label="สาขาวิชา"
                      value={data.profile.major}
                    />
                    <InfoRow
                      icon={<Mail size={9} fill="#173b69" strokeWidth={1} />}
                      label="อีเมล"
                      value={data.profile.email}
                    />
                    <InfoRow
                      icon={<Phone size={9} fill="#173b69" strokeWidth={1} />}
                      label="โทรศัพท์"
            ...
            <section className="skills-grid">
              <div className="transcript-card skills-card">
                <div className="flex items-center justify-between border-b border-[#e3e6eb] pb-[5px]">
                  <div className="flex items-center gap-[5px]">
                    <BarChart3 size={14} strokeWidth={2.2} className="text-[#173b69]" />
                    <h2 className="text-[10px] font-bold text-[#173b69]">คะแนนรวมทักษะ</h2>
                  </div>
                  <span className="text-[8px] font-bold text-[#173b69]">%</span>
                </div>

                <div className="mt-[8px] space-y-[5px]">
                  {data.skillsBars.map((skill) => (
                    <SkillProgress
                      key={skill.name}
                      name={skill.name}
                      percent={skill.percent}
              ...
              <div className="transcript-card skills-card">
                <div className="flex items-center justify-between border-b border-[#e3e6eb] pb-[5px]">
                  <div className="flex items-center gap-[5px]">
                    <BarChart3 size={14} strokeWidth={2.2} className="text-[#173b69]" />
                    <h2 className="text-[10px] font-bold text-[#173b69]">ทักษะ</h2>
                  </div>
                  <span className="text-[9px] font-bold text-[#173b69]">คะแนนประเมิน</span>
                </div>

                <div className="mt-[7px] space-y-[5px]">
                  {data.skillsScores.map((skill) => (
                    <div
                      key={skill.name}
            ...
            <section className="transcript-card activity-card">
              <div className="section-title">
                <CalendarDays
                  ...
                  fill="#dce6f4"
                />
                <h2 className="text-[10px] font-bold text-[#173b69]">กิจกรรมที่เข้าร่วม</h2>
              </div>
            ...
            <footer className="mt-[7px] border-t-2 border-[#587596] pt-[5px] text-center">
              <div className="flex justify-center items-center gap-[6px]">
                ...
                <p className="text-[7.5px] text-[#173b69]">
                  วันที่ออกเอกสาร : {data.dateIssued}
                </p>
              </div>
...
function InfoRow({
  ...
  return (
    <div className="flex items-start gap-[5px] text-[#555]">
      <span className="text-[#173b69] shrink-0 mt-[1px]">{icon}</span>
      {inline ? (
        <div className="flex items-baseline gap-[4px]">
          <span className="text-[#3f6ea8] text-[6px] leading-[7px]">{label}</span>
          <span className="text-[#444] text-[6px] leading-[7px]">{value}</span>
        </div>
      ) : (
