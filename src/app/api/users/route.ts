...
export async function GET() {
  try {
    const query = `
      SELECT 
        u.userId, u.email, u.role, u.status, u.created_at, u.updated_at, u.must_change_password,
      ...
      GROUP BY u.userId, u.email, u.role, u.status, u.created_at, u.updated_at, u.must_change_password,
    ...
    const users = rows.map((row) => {
      ...
      return {
        ...
        advisorUserIds: row.advisorUserIds ? String(row.advisorUserIds).split(",").filter(Boolean) : [],
        advisorName: advisorNames.length > 0 ? advisorNames.join(", ") : null,
...
// POST: เพิ่มผู้ใช้ใหม่ (รหัสผ่านเริ่มต้น SkillSciDI)
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // Insert users with must_change_password = 1
      await connection.query(
        `INSERT INTO users (userId, email, password, role, status, must_change_password)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [userId, email, passwordHash, role, status, 1]
      );

      // Insert role-specific
      if (role === "student") {
        if (!studentId) throw httpError(400, "กรุณากรอกรหัสนิสิต");
        await connection.query(
          `INSERT INTO students (studentId, userId, firstname, lastname, faculty, major, year, phone)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [studentId, userId, firstName, lastName, faculty || null, major || null, year ? Number(year) : null, phone || null]
        );

        if (advisorUserIds && Array.isArray(advisorUserIds) && advisorUserIds.length > 0) {
        ...
        await connection.query(
          `INSERT INTO teacher (teacherId, userId, firstname, lastname, position, faculty, program, isExecutive)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [nanoid(20), userId, firstName, lastName, position || null, faculty || null, program || null, isExec]
        );
      } else if (role === "officer") {
        await connection.query(
          `INSERT INTO officer (officerId, userId, firstname, lastname, position, faculty)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [nanoid(20), userId, firstName, lastName, position || null, faculty || null]
        );
      } else {
        throw httpError(400, "บทบาทไม่ถูกต้อง (รองรับเฉพาะ student, teacher, officer)");
      }
    ...
    if (
      ...
      return jsonError(httpError(409, "ข้อมูลซ้ำ (อาจจะรหัสนิสิตซ้ำ)"));
    }
    return jsonError(error);
  }
}