  ...
  try {
    ...
    if (
      ...
      if (files.length === 0) {
        throw httpError(
          400,
          "กรุณาแนบเอกสารหลักฐานอย่างน้อย 1 ไฟล์",
        );
      }

      const requestUploadId =
        nanoid(10);
...
return {
  name: file.name,
  url: `/api/activity-requests/evidence/${requestUploadId}/${encodeURIComponent(
    fileName,
  )}`,
  type: file.type || null,
};
    ...
    if (
      ...
      throw httpError(
        400,
        "วันที่สิ้นสุดกิจกรรมต้องไม่ก่อนวันที่เริ่มกิจกรรม",
      );
    }
